...
5. Run `blender -b tools/construction-film/sce-construction.blend -t 6 -P tools/construction-film/day_cycle.py -- <render-directory> render`. This freezes the completed campus and writes `energy-day-cycle.blend` plus 624 native frames in `day-native24/`. The 26-second cycle is rendered directly at 24 fps, with no synthesized in-between frames. Use `preview` to render five lighting checks first. To upgrade an existing 12 fps render, load its `energy-day-cycle.blend` and run `render_native_day.py -- <render-directory>`; it reuses the existing odd frames and renders the missing even frames.
6. Run `python tools/construction-film/encode_day_cycle.py --construction <original-frames> --day <render-directory>/day-native24 --out resources/media/green-vision`. This preserves the first 36 seconds, appends the native cycle and creates the 62-second desktop/mobile films. The H.264 files use two-second keyframes and bounded bitrates (6 Mbps desktop / 1.5 Mbps mobile) to limit decoding and transfer spikes. `encode.py` remains available to reproduce the earlier construction-only film and its posters.
...
The video and poster have a fixed size and crop throughout playback. The previous entry/reset resizing and panning have been removed entirely, including their per-frame JavaScript callbacks. All camera motion is now inside the native 24 fps film. The information card uses a translucent solid background rather than blurring the moving video. `timeupdate` updates only text and battery state; unchanged text is not rewritten, and completed environmental counters update only when the language changes. `qa_motion.cjs` checks both the solar entrance and reset for zero media-layout changes and measures playback cadence on desktop/mobile.
