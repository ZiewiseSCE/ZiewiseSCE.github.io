...
7. Preview through an HTTP server with byte-range support, such as `node tools/green-vision/serve.cjs`. Check muted looping, sun visibility, moving shadows, ESS flow direction, signage, service-route pause/resume, reduced motion, three languages and 320–2560 px widths. Run `qa.cjs` and `qa_energy.cjs` with `NODE_PATH` resolving Playwright.
